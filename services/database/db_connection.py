...
class UserInterface(BaseQueriesMixin):
    ...
    def save_book(self, user_id: int, book_name: str, content: str) -> None:
        if book_name not in self.get_books(user_id):
...
# book = Book('./books/book.txt')
# json_content = book.get_json_text()
#
# bot_database.user_interface.save_book(851605033, 'test', json_content)
# print(bot_database.book_interface.get_content('book')['12'])
